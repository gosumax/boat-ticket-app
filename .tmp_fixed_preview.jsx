...
const DispatcherShiftClose = ({ setShiftClosed: setGlobalShiftClosed }) => {
  ...
  const refreshSummary = async (businessDay) => {
    ...
    try {
      ...
      const msg = e?.response?.error || e?.response?.message || e?.message || '� удалось загрузить данные смены';
      setLoadError(msg);
      setLoading(false);
    } finally {
      setReloading(false);
  ...
  const fillAllDrafts = () => {
    ...
    for (const s of sellers) {
      ...
      next[sellerId] = {
        // Заполнять только если: due > 0 �? поле пустое
  ...
  }, [normalizedSummary, ownerCashAvailable, futureTripsReserveCash]);
  ...
  const fundsUi = useMemo(() => {
    ...
    const seasonFromPrepaymentTransfer = Number(
      withhold.season_from_prepayment_transfer ?? withhold.season_amount_from_cancelled_prepayment ?? 0
    );
    ...
    return {
      weeklyAmount: Number(withhold.weekly_amount ?? 0),
      dispatcherAmount: Number(withhold.dispatcher_amount_total ?? 0),
      seasonFromRevenue,
      seasonFromPrepaymentTransfer,
      roundingToSeason: Number(withhold.rounding_to_season_amount_total ?? 0),
      seasonTotal,
      fundTotalOriginal: Number(withhold.fund_total_original ?? 0),
      fundTotalAfterWithhold: Number(withhold.fund_total_after_withhold ?? 0),
    };
  }, [normalizedSummary]);

  const cashDiscrepancyHint = useMemo(() => {
    if (allTripsFinished) {
      return 'Закрытие смены разрешено, но проверь кассу перед закрытием.';
    }
    return `Есть незавершённые рейсы: ${openTripsCount}. Операции по смене заблокированы до завершения рейсов.`;
  }, [allTripsFinished, openTripsCount]);
  ...
  const applySalaryPayoutForSeller = async (sellerId) => {
    ...
    try {
      ...
      alert('� удалось провести выплату зарплаты. �оверь соединение и попробуй снова.');
    }
  };
  ...
  const applyCashDeposit = async (sellerId) => {
    ...
    try {
      ...
      if (result.handled) return;
      alert('� удалось провести сдачу наличных. �оверь соединение и попробуй снова.');
    }
  };
  ...
  const applyTerminalClose = async (sellerId) => {
    ...
    try {
      ...
      if (result.handled) return;
      alert('� удалось закрыть терминальный долг. �оверь соединение и попробуй снова.');
    }
  };
  ...
  const handleShiftClose = async () => {
    ...
    if (salarySummary.remaining > 0) {
      alert('�льзя закрыть смену: есть невыплаченная зарплата.');
      return;
    }
    if (debtSummary.total > 0) {
      alert('�льзя закрыть смену: есть несданные суммы у продавцов.');
      return;
    }
    ...
    try {
      ...
      if (status === 400) {
        ...
        alert(`�льзя закрыть смену: ${msg}`);
        return;
      }

      // Other errors
      console.error('shift close failed', e);
      const msg = errData?.error || errData?.message || '�оверь соединение и повтори.';
        ...
        <div className="p-3">
          <div className="max-w-4xl mx-auto">
            <div className="bg-neutral-900 rounded-2xl p-4 text-center">
              <div className="text-red-300 mb-3">� удалось загрузить данные смены.</div>
              <button
                type="button"
        ...
        <div className="max-w-4xl lg:max-w-5xl xl:max-w-6xl mx-auto space-y-6">
          ...
          <div className="bg-neutral-900 rounded-2xl  p-3">
            ...
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-xl font-bold text-neutral-100">�?ТОГО ЗА ДЕ�</h2>
            ...
            {shiftClosed && closedAt && (
              <div className="mb-3 p-2 bg-green-950/50 border border-green-900/50 rounded-lg text-sm text-green-300 text-center">
                �ена закрыта в {closedAt}{closedBy ? ` (id: ${closedBy})` : ''}. Операции запрещены.
              </div>
            )}
            {loadError ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div className="flex items-center justify-between py-2 px-3 bg-neutral-900 rounded-lg">
                  ...
                  <span data-testid="shiftclose-withhold-season" className="text-lg font-bold text-purple-300">
                    {formatRUB(fundsUi?.seasonTotal ?? 0)}
                  </span>
                </div>
                <div className="flex items-center justify-between py-2 px-3 bg-neutral-900 rounded-lg">
                ...
                <div className="flex items-center justify-between py-2 px-3 bg-neutral-900 rounded-lg">
                  <span className="text-neutral-400">�ать owner наличными сегодня</span>
                ...
                <div className="flex items-center justify-between py-2 px-3 bg-neutral-900 rounded-lg">
                  <span className="text-neutral-400">�брать с продавцов</span>
            ...
            {fundsUi && (
              <details className="mt-3 rounded-lg border border-neutral-800 bg-neutral-900/40 p-3">
                <summary className="cursor-pointer text-sm text-neutral-300">Debug details (internal calculations)</summary>
                <div className="mt-2 space-y-2 text-xs text-neutral-400">
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <span>Funds withhold cash today</span>
                    <span className="font-semibold text-purple-300">{formatRUB(fundsWithholdCashToday)}</span>
                  </div>
                  <div className="flex items-center justify-between gap-3">
                    <span>Season from revenue / prepayment transfer</span>
                    <span className="font-semibold">{formatRUB(fundsUi.seasonFromRevenue)} / {formatRUB(fundsUi.seasonFromPrepaymentTransfer)}</span>
                  </div>
                </div>
              </details>
            )}
          ...
          {(cashDiscrepancy !== 0 || cashboxWarnings.length > 0) && (
            <div className={`mt-3 rounded-lg border px-3 py-2 text-left text-sm ${
              cashDiscrepancy > 0 
                ? 'bg-yellow-950/20 border-yellow-900/40' 
                : 'bg-red-950/20 border-red-900/40'
            }`}>
              <div className={`font-semibold ${cashDiscrepancy > 0 ? 'text-yellow-300' : 'text-red-300'}`}>
                Диагностика: расхождение кассы
              </div>
          ...
          <div
            data-testid="shiftclose-close-status"
          ...
          >
            {canCloseShift
              ? 'Статус: смену можно закрывать.'
              : `Статус: закрытие смены заблокировано${closeBlockReason ? ` (${closeBlockReason})` : '.'}`}
          </div>
          </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b">
                    <th className="text-left py-2 text-neutral-400 font-medium">Seller</th>
                    <th className="text-left py-2 text-neutral-400 font-medium">Status</th>
                    <th className="text-right py-2 text-neutral-400 font-medium">Cash due</th>
                    <th className="text-right py-2 text-neutral-400 font-medium">Card due</th>
                    <th className="text-right py-2 text-neutral-400 font-medium">Total due</th>
                    <th className="text-right py-2 text-neutral-400 font-medium">Details</th>
                  </tr>
                </thead>
                <tbody>
                  {(() => {
                    // �?сточник продавцов: normalizedSummary.sellers (нормализованные) -> sellersData (legacy state) -> пусто
                    const sellersForRender = normalizedSummary?.sellers ?? sellersData ?? [];
                    if (sellersForRender.length === 0) {
                      return (
                        <tr>
                          <td colSpan={6} className="py-4 text-center text-neutral-500" data-testid="shiftclose-sellers-empty">
                            �т данных по продавцам за этот день
                          </td>
                        </tr>
                      );
                    }
                      ...
                      return (
                        <Fragment key={s.id}>
                          <tr className="border-b hover:bg-neutral-950" data-testid={`shiftclose-seller-row-${s.id}`}>
                            ...
                            <td data-testid={`shiftclose-seller-total-due-${s.id}`} className={`text-right py-3 ${totalRem > 0 ? 'text-red-300 font-semibold' : 'text-neutral-200'}`}>{formatRUB(totalRem)}</td>
                                ...
                                <div className="bg-neutral-950 border border-neutral-800 rounded-2xl p-3">
                                  ...
                                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    ...
                                    <div className="bg-neutral-900/40 border border-neutral-800 rounded-xl p-3">
                                      <div className="text-sm text-neutral-200 font-semibold mb-2">Money Movement</div>

                                      <div className="mb-3 grid grid-cols-3 gap-2 rounded-xl border border-neutral-800 bg-neutral-950/70 p-3 text-center">
                                        <div>
                                          <div className="text-[11px] uppercase tracking-wide text-neutral-500">Cash due</div>
                                          <div className={`mt-1 font-semibold ${cashRem > 0 ? 'text-red-300' : 'text-neutral-200'}`}>{formatRUB(cashRem)}</div>
                                        </div>
                                        <div>
                                          <div className="text-[11px] uppercase tracking-wide text-neutral-500">Card due</div>
                                          <div className={`mt-1 font-semibold ${termRem > 0 ? 'text-red-300' : 'text-neutral-200'}`}>{formatRUB(termRem)}</div>
                                        </div>
                                        <div>
                                          <div className="text-[11px] uppercase tracking-wide text-neutral-500">Total due</div>
                                    ...
                                    <div className="bg-orange-950/20 border border-orange-900/30 rounded-xl p-3">
                                      ...
                                      {/* �числено - only show real per-seller data, no fake calculations */}
                                      <div className="mb-2 pb-2 border-b border-neutral-800">
                                        <div className="text-xs text-neutral-400">�числено сегодня:</div>
                                        {(() => {
          ...
          {shiftClosed ? (
            <div className="bg-green-950/30 border border-green-900/50 rounded-2xl p-3 mt-6">
              ...
              <p className="text-green-400 font-medium">�ена закрыта</p>
            </div>
          ) : (
            <>
              {/* Confirmation Checks */}
                ...
                <div className="space-y-2">
                  ...
                  <label className="flex items-center space-x-3">
                    <input
                      ...
                      className="h-5 w-5 text-purple-600 rounded focus:ring-purple-500"
                    />
                    <span className="text-neutral-300">�етензий по рейсам и билетам нет</span>
                  </label>
                </div>
              </div>
              ...
              {/* Action Buttons */}
              {closeBlockReason ? (
                <div className="text-sm text-red-400 text-center mt-3">�льзя закрыть смену: {closeBlockReason}</div>
              ) : null}
