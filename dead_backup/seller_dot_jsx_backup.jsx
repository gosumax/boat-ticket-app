...
const SelectSeats = ({ 
  ...
  const durationLabel = (() => {
    if (!trip) return '';
    if (trip.duration_minutes != null) return formatDuration(trip.duration_minutes);
    if (trip.durationMinutes != null) return formatDuration(trip.durationMinutes);
    if (trip.duration_min != null) return formatDuration(trip.duration_min);
    if (trip.duration_hours != null) return `${trip.duration_hours} ч`;
    if (trip.durationHours != null) return `${trip.durationHours} ч`;
    if (typeof trip.duration === 'string' && trip.duration.trim()) return trip.duration;
    return '';
  })();
  ...
  return (
    <div className="flex flex-col">
      <h2 className="text-2xl font-extrabold text-gray-900 mb-6 text-center tracking-tight">Количество мест</h2>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 mb-6">
        <h3 className="font-bold text-lg mb-2"><span className="text-gray-900 font-semibold">Детали рейса</span></h3>
        <p className="text-gray-600">{trip?.boat_name} • {trip?.time}{durationLabel ? ` • Длительность: ${durationLabel}` : ''}</p>
        ...
        <div className="space-y-4">
          <div className={`grid ${isBananaTrip ? "grid-cols-2" : "grid-cols-3"} gap-6 justify-items-center`}>
            <div className="text-center">
              <p className="text-sm font-semibold text-gray-800 uppercase tracking-wide">Взрослый</p>
              <div className="flex items-center justify-center gap-3 mt-3 w-[160px]">
                <button 
                  type="button"
                  onClick={() => decrementTicket('adult')}
                  className="w-10 h-10 rounded-xl bg-white border border-gray-300 shadow-sm flex items-center justify-center text-2xl font-extrabold text-blue-700 hover:bg-gray-50 active:scale-[0.98] transition disabled:opacity-40 disabled:cursor-not-allowed"
                  disabled={ticketBreakdown.adult <= 0}
                >
                  -
                </button>
                <span className="mx-3 text-2xl font-extrabold text-gray-900 tabular-nums">{ticketBreakdown.adult}</span>
                <button 
                  type="button"
            ...
            <div className="text-center">
              <p className="text-sm font-semibold text-gray-800 uppercase tracking-wide">Подросток</p>
              <div className="flex items-center justify-center gap-3 mt-3 w-[160px]">
                <button 
                  type="button"
                  onClick={() => decrementTicket('teen')}
                  className={`w-10 h-10 rounded-xl bg-white border border-gray-300 shadow-sm flex items-center justify-center text-2xl font-extrabold text-blue-700 hover:bg-gray-50 active:scale-[0.98] transition disabled:opacity-40 disabled:cursor-not-allowed rounded-full flex items-center justify-center text-xl font-bold hover:bg-gray-300 active:bg-gray-400 ${trip?.boat_type === 'banana' ? 'opacity-50 cursor-not-allowed' : ''}`}
                  disabled={ticketBreakdown.teen <= 0 || trip?.boat_type === 'banana'}
                >
                  -
                </button>
                <span className="mx-3 text-2xl font-extrabold text-gray-900 tabular-nums">{ticketBreakdown.teen}</span>
                <button 
                  type="button"
              ...
              {trip?.boat_type === 'banana' && (
                <p className="text-xs text-red-600 mt-1">Недоступно для банана</p>
                        ...
                        <div className="text-center">
              <p className="text-sm font-semibold text-gray-800 uppercase tracking-wide">Детский</p>
              <div className="flex items-center justify-center gap-3 mt-3 w-[160px]">
                <button 
                  type="button"
                ...
                <span className="mx-3 text-2xl font-extrabold text-gray-900 tabular-nums">{ticketBreakdown.child}</span>
                <button 
                  type="button"
          ...
          <div className="text-center font-extrabold text-lg text-gray-900">
            <span className="text-gray-900 font-semibold">Итого:</span> <span className="inline-flex items-center px-3 py-1 rounded-full bg-blue-50 text-blue-800 border border-blue-100 tabular-nums">{ticketBreakdown.adult + ticketBreakdown.teen + ticketBreakdown.child}</span> мест
          </div>
        </div>
...
{!isBananaTrip && (
            <div className="flex justify-between mt-1">
              <span className="text-sm text-gray-700">Подросток:</span>
              <span className="font-medium">{formatRUB(trip?.price_teen || trip?.price)}</span>
            </div>
          )}
          <div className="flex justify-between mt-1">
            <span className="text-sm text-gray-700">Детский:</span>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 mb-6">
        <h3 className="font-bold text-lg mb-4"><span className="text-gray-900 font-semibold">Информация о клиенте</span></h3>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 mb-6">
        <div className="flex justify-between items-center">
          <span className="font-bold text-lg"><span className="text-gray-900 font-semibold">Итого:</span></span>
