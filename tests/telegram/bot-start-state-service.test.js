import Database from 'better-sqlite3';
import { beforeEach, describe, expect, it } from 'vitest';
import { createTelegramPersistenceContext } from '../../server/telegram/index.js';
import {
  TELEGRAM_BOT_START_ACTIONS,
  TELEGRAM_BOT_START_MODES,
  TELEGRAM_BOT_START_RESPONSE_VERSION,
} from '../../shared/telegram/index.js';

const TELEGRAM_TABLES = Object.freeze([
  'telegram_guest_profiles',
  'telegram_traffic_sources',
  'telegram_source_qr_codes',
  'telegram_seller_attribution_sessions',
  'telegram_guest_entries',
  'telegram_booking_requests',
  'telegram_booking_holds',
  'telegram_booking_request_events',
  'telegram_content_blocks',
  'telegram_notifications',
  'telegram_analytics_events',
  'telegram_post_trip_messages',
  'telegram_post_trip_offers',
]);

function createTestDb() {
  const db = new Database(':memory:');
  db.pragma('foreign_keys = ON');
  db.exec(`
    CREATE TABLE settings (key TEXT PRIMARY KEY, value TEXT);
    CREATE TABLE users (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      username TEXT,
      role TEXT,
      is_active INTEGER NOT NULL DEFAULT 1
    );
    CREATE TABLE presales (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      boat_slot_id INTEGER NULL,
      status TEXT DEFAULT 'ACTIVE',
      slot_uid TEXT NULL,
      business_day TEXT NULL
    );
    CREATE TABLE tickets (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      presale_id INTEGER NOT NULL REFERENCES presales(id),
      boat_slot_id INTEGER NULL,
      status TEXT NOT NULL DEFAULT 'ACTIVE'
    );
  `);
  db.prepare(`INSERT INTO users (id, username, role, is_active) VALUES (1, 'seller-a', 'seller', 1)`).run();
  return db;
}

function createClock(startIso) {
  let current = new Date(startIso);
  return {
    now: () => new Date(current),
    advanceMinutes(minutes) {
      current = new Date(current.getTime() + minutes * 60 * 1000);
    },
  };
}

function wireClock(context, clock) {
  context.services.attributionService.now = clock.now;
  context.services.bookingRequestService.now = clock.now;
}

function snapshotTelegramRowCounts(db) {
  return Object.fromEntries(
    TELEGRAM_TABLES.map((tableName) => [
      tableName,
      db.prepare(`SELECT COUNT(*) AS count FROM ${tableName}`).get().count,
    ])
  );
}

function seedGuest(context, suffix) {
  return context.repositories.guestProfiles.create({
    telegram_user_id: `tg-bot-start-${suffix}`,
    display_name: `Bot Start Guest ${suffix}`,
    username: `bot_start_${suffix}`,
    language_code: 'ru',
    phone_e164: `+7999888${suffix}`,
    consent_status: 'granted',
    profile_status: 'active',
  });
}

function seedAttribution(context, guest, suffix) {
  const source = context.repositories.trafficSources.create({
    source_code: `bot-start-source-${suffix}`,
    source_type: 'seller_qr',
    source_name: `Bot Start Source ${suffix}`,
    default_seller_id: 1,
    is_active: 1,
  });
  const qr = context.repositories.sourceQRCodes.create({
    qr_token: `bot-start-token-${suffix}`,
    traffic_source_id: source.traffic_source_id,
    seller_id: 1,
    entry_context: { zone: `bot-start-zone-${suffix}` },
    is_active: 1,
  });

  return context.services.attributionService.registerGuestEntryFromSource({
    guest_profile_id: guest.guest_profile_id,
    traffic_source_id: source.traffic_source_id,
    source_qr_code_id: qr.source_qr_code_id,
    entry_channel: 'qr',
  }).sellerAttributionSession;
}

function seedRequest(context, clock, suffix) {
  const guest = seedGuest(context, suffix);
  const attribution = seedAttribution(context, guest, suffix);

  clock.advanceMinutes(1);
  const lifecycle = context.services.bookingRequestService.createBookingRequest({
    guest_profile_id: guest.guest_profile_id,
    seller_attribution_session_id: attribution.seller_attribution_session_id,
    requested_trip_date: '2026-04-11',
    requested_time_slot: '12:00',
    requested_seats: 2,
    requested_ticket_mix: { adult: 2 },
    requested_prepayment_amount: 1000,
    currency: 'RUB',
    contact_phone_e164: guest.phone_e164,
  });

  return {
    guest,
    attribution,
    bookingRequestId: lifecycle.bookingRequest.booking_request_id,
  };
}

describe('telegram bot start-state service', () => {
  let db;
  let context;
  let clock;

  beforeEach(() => {
    db = createTestDb();
    clock = createClock('2026-04-10T10:00:00.000Z');
    context = createTelegramPersistenceContext(db);
    wireClock(context, clock);
  });

  it('returns new_guest mode by telegram-user reference for guest with no request history', () => {
    const guest = seedGuest(context, '1001');
    const beforeCounts = snapshotTelegramRowCounts(db);

    const state =
      context.services.botStartStateService.readBotStartStateByTelegramUserReference({
        telegram_user_reference: {
          reference_type: 'telegram_user',
          telegram_user_id: guest.telegram_user_id,
        },
      });

    expect(state).toMatchObject({
      response_version: TELEGRAM_BOT_START_RESPONSE_VERSION,
      read_only: true,
      start_mode: TELEGRAM_BOT_START_MODES.new_guest,
      primary_guest_state: 'new_guest',
      telegram_user_summary: {
        guest_profile_id: guest.guest_profile_id,
        telegram_user_id: guest.telegram_user_id,
        display_name: 'Bot Start Guest 1001',
      },
      current_route_summary: null,
      active_booking_request_summary: null,
      latest_ticket_presale_linkage_summary: null,
      recommended_next_actions: [
        TELEGRAM_BOT_START_ACTIONS.view_trips,
        TELEGRAM_BOT_START_ACTIONS.create_booking_request,
        TELEGRAM_BOT_START_ACTIONS.contact,
        TELEGRAM_BOT_START_ACTIONS.faq,
        TELEGRAM_BOT_START_ACTIONS.useful_content,
      ],
      latest_timestamp_summary: {
        iso: expect.any(String),
        unix_seconds: expect.any(Number),
      },
      primary_actions: [TELEGRAM_BOT_START_ACTIONS.view_trips],
      secondary_actions: [
        TELEGRAM_BOT_START_ACTIONS.create_booking_request,
        TELEGRAM_BOT_START_ACTIONS.contact,
        TELEGRAM_BOT_START_ACTIONS.faq,
        TELEGRAM_BOT_START_ACTIONS.useful_content,
      ],
      requested_booking_request_id: null,
    });
    expect(Object.isFrozen(state)).toBe(true);
    expect(Object.isFrozen(state.telegram_user_summary)).toBe(true);
    expect(Object.isFrozen(state.recommended_next_actions)).toBe(true);
    expect(snapshotTelegramRowCounts(db)).toEqual(beforeCounts);
  });

  it('returns active_request mode with active booking summary and seller route summary', () => {
    const seeded = seedRequest(context, clock, '2002');
    const beforeCounts = snapshotTelegramRowCounts(db);

    const state =
      context.services.botStartStateService.readBotStartStateByBookingRequestReference({
        booking_request_reference: {
          reference_type: 'telegram_booking_request',
          booking_request_id: seeded.bookingRequestId,
        },
      });

    expect(state).toMatchObject({
      start_mode: TELEGRAM_BOT_START_MODES.active_request,
      primary_guest_state: 'active_request_waiting_for_prepayment',
      current_route_summary: {
        route_target_type: 'seller',
        seller_reference: {
          seller_id: 1,
          seller_attribution_session_id: seeded.attribution.seller_attribution_session_id,
        },
      },
      active_booking_request_summary: {
        booking_request_reference: {
          booking_request_id: seeded.bookingRequestId,
        },
        booking_request_id: seeded.bookingRequestId,
        request_status: 'HOLD_ACTIVE',
        requested_trip_date: '2026-04-11',
        requested_time_slot: '12:00',
        requested_seats: 2,
        requested_ticket_mix: { adult: 2 },
        contact_phone_e164: seeded.guest.phone_e164,
        confirmed_presale_id: null,
        hold_status: 'ACTIVE',
        hold_expires_at: '2026-04-10T10:16:00.000Z',
      },
      latest_ticket_presale_linkage_summary: null,
      recommended_next_actions: [
        TELEGRAM_BOT_START_ACTIONS.view_current_request,
        TELEGRAM_BOT_START_ACTIONS.contact,
        TELEGRAM_BOT_START_ACTIONS.faq,
        TELEGRAM_BOT_START_ACTIONS.useful_content,
      ],
      primary_actions: [TELEGRAM_BOT_START_ACTIONS.view_current_request],
    });
    expect(Object.isFrozen(state.active_booking_request_summary)).toBe(true);
    expect(snapshotTelegramRowCounts(db)).toEqual(beforeCounts);
  });

  it('returns linked_ticket mode with latest ticket/presale linkage summary', () => {
    const seeded = seedRequest(context, clock, '3003');
    const confirmed = context.services.bookingRequestService.confirmPrepayment(
      seeded.bookingRequestId,
      {
        actorType: 'system',
        actorId: 'payment-3003',
      }
    );
    const presaleId = db.prepare(`
      INSERT INTO presales (boat_slot_id, status, slot_uid, business_day)
      VALUES (42, 'ACTIVE', 'generated:42', '2026-04-11')
    `).run().lastInsertRowid;
    db.prepare(`
      INSERT INTO tickets (presale_id, boat_slot_id, status)
      VALUES
        (?, 42, 'ACTIVE'),
        (?, 42, 'USED')
    `).run(presaleId, presaleId);
    context.repositories.bookingRequests.updateById(confirmed.bookingRequest.booking_request_id, {
      request_status: 'CONFIRMED_TO_PRESALE',
      confirmed_presale_id: presaleId,
      last_status_at: '2026-04-10T10:03:00.000Z',
    });
    const beforeCounts = snapshotTelegramRowCounts(db);

    const state =
      context.services.botStartStateService.readBotStartStateByTelegramUserReference({
        telegram_user_reference: {
          reference_type: 'telegram_user',
          telegram_user_id: seeded.guest.telegram_user_id,
        },
      });

    expect(state).toMatchObject({
      start_mode: TELEGRAM_BOT_START_MODES.linked_ticket,
      primary_guest_state: 'guest_with_linked_ticket',
      active_booking_request_summary: null,
      latest_ticket_presale_linkage_summary: {
        booking_request_reference: {
          booking_request_id: seeded.bookingRequestId,
        },
        booking_request_id: seeded.bookingRequestId,
        request_status: 'CONFIRMED_TO_PRESALE',
        confirmed_presale_id: presaleId,
        linkage_source: 'telegram_booking_request.confirmed_presale_id',
        ticket_status: 'TICKET_READY',
        ticket_state_group: 'linked_to_presale',
        canonical_linkage_status: 'enriched',
        canonical_presale: {
          exists: true,
          presale_id: presaleId,
          status: 'ACTIVE',
        },
        linked_ticket_summary: {
          read_status: 'readable',
          total_count: 2,
          status_counts: [
            { status: 'ACTIVE', count: 1 },
            { status: 'USED', count: 1 },
          ],
        },
        trip_linkage_summary: {
          derivation_status: 'derived',
          derivable: true,
          derivation_source: 'canonical_presale_and_tickets',
          slot_uid: 'generated:42',
          boat_slot_id: 42,
          business_day: '2026-04-11',
          inconsistency_reasons: [],
        },
      },
      recommended_next_actions: [
        TELEGRAM_BOT_START_ACTIONS.view_ticket,
        TELEGRAM_BOT_START_ACTIONS.view_trips,
        TELEGRAM_BOT_START_ACTIONS.contact,
        TELEGRAM_BOT_START_ACTIONS.faq,
        TELEGRAM_BOT_START_ACTIONS.useful_content,
      ],
      primary_actions: [TELEGRAM_BOT_START_ACTIONS.view_ticket],
    });
    expect(Object.isFrozen(state.latest_ticket_presale_linkage_summary)).toBe(true);
    expect(
      Object.isFrozen(
        state.latest_ticket_presale_linkage_summary.linked_ticket_summary.status_counts[0]
      )
    ).toBe(true);
    expect(snapshotTelegramRowCounts(db)).toEqual(beforeCounts);
  });

  it('returns completed_guest_without_active_request when history exists but no active request', () => {
    const seeded = seedRequest(context, clock, '4004');
    clock.advanceMinutes(16);
    context.services.bookingRequestService.expireHold(seeded.bookingRequestId, {
      actorType: 'system',
      actorId: 'hold-expirer',
    });

    const state =
      context.services.botStartStateService.readBotStartStateByTelegramUserReference({
        telegram_user_reference: {
          reference_type: 'telegram_user',
          telegram_user_id: seeded.guest.telegram_user_id,
        },
      });

    expect(state).toMatchObject({
      start_mode: TELEGRAM_BOT_START_MODES.completed_guest_without_active_request,
      primary_guest_state: 'guest_completed_without_active_request',
      active_booking_request_summary: null,
      latest_ticket_presale_linkage_summary: null,
      recommended_next_actions: [
        TELEGRAM_BOT_START_ACTIONS.view_trips,
        TELEGRAM_BOT_START_ACTIONS.create_booking_request,
        TELEGRAM_BOT_START_ACTIONS.contact,
        TELEGRAM_BOT_START_ACTIONS.faq,
        TELEGRAM_BOT_START_ACTIONS.useful_content,
      ],
    });
  });

  it('rejects invalid references deterministically', () => {
    expect(() =>
      context.services.botStartStateService.readBotStartStateByTelegramUserReference({
        telegram_user_reference: {
          reference_type: 'telegram_chat',
          telegram_user_id: 'u1',
        },
      })
    ).toThrow('[TELEGRAM_BOT_START_STATE] Unsupported telegram-user reference type');

    expect(() =>
      context.services.botStartStateService.readBotStartStateByBookingRequestReference({
        booking_request_reference: {
          reference_type: 'telegram_booking_request',
          booking_request_id: 0,
        },
      })
    ).toThrow(
      '[TELEGRAM_BOT_START_STATE] booking_request_reference.booking_request_id must be a positive integer'
    );
  });
});
