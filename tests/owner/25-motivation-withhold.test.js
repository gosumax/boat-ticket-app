...
describe('MOTIVATION WITHHOLD', () => {
  ...
  describe('A) Withhold fields exist and have correct structure', () => {
    ...
    it('returns withhold object with all expected fields', async () => {
      ...
      expect(typeof res.body.data.withhold.season_amount_base).toBe('number');
      expect(typeof res.body.data.withhold.rounding_to_season_amount_total).toBe('number');
      expect(typeof res.body.data.withhold.weekly_rounding_to_season_amount).toBe('number');
      expect(typeof res.body.data.withhold.dispatcher_rounding_to_season_amount).toBe('number');
      expect(typeof res.body.data.withhold.payouts_rounding_to_season_amount).toBe('number');
  ...
  describe('B) Withhold calculation with revenue', () => {
    ...
    it('routes all rounding remainders to season fund', async () => {
      ...
      // Non-round values to force weekly/dispatcher/payout remainders.
      db.prepare(`
        INSERT INTO money_ledger (kind, type, amount, status, seller_id, business_day, event_time)
        VALUES ('SELLER_SHIFT', 'SALE_ACCEPTED_CASH', 123457, 'POSTED', ?, ?, datetime('now'))
      `).run(sellerId, DAY1);
      if (dispatcherId) {
        db.prepare(`
          INSERT INTO money_ledger (kind, type, amount, status, seller_id, business_day, event_time)
          VALUES ('DISPATCHER_SHIFT', 'SALE_ACCEPTED_CASH', 23457, 'POSTED', ?, ?, datetime('now'))
        `).run(dispatcherId, DAY1);
      }
      ...
      expect(res.status).toBe(200);
      const w = res.body.data.withhold;

      expect(w.season_amount).toBeCloseTo(w.season_amount_base + w.rounding_to_season_amount_total, 6);
      expect(w.rounding_to_season_amount_total).toBeCloseTo(
        w.weekly_rounding_to_season_amount +
        w.dispatcher_rounding_to_season_amount +
        w.payouts_rounding_to_season_amount,
        6
      );
