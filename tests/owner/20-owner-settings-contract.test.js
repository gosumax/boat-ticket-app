...
describe('OWNER SETTINGS CONTRACT', () => {
  ...
  describe('F) Season MM-DD boundaries (optional settings fields)', () => {

    it('persists season_start_mmdd and season_end_mmdd when format is valid', async () => {
      const putRes = await request(app)
        .put('/api/owner/settings/full')
        .set('Authorization', `Bearer ${ownerToken}`)
        .send({ season_start_mmdd: '05-01', season_end_mmdd: '10-15' });

      expect(putRes.status).toBe(200);
      expect(putRes.body.ok).toBe(true);
      expect(putRes.body.data.season_start_mmdd).toBe('05-01');
      expect(putRes.body.data.season_end_mmdd).toBe('10-15');
    ...
    it('returns 400 when MM-DD format is invalid', async () => {
      const putRes = await request(app)
        .put('/api/owner/settings/full')
        .set('Authorization', `Bearer ${ownerToken}`)
        .send({ season_start_mmdd: '5-01', season_end_mmdd: '10-15' });

      expect(putRes.status).toBe(400);
      expect(putRes.body.ok).toBe(false);
      expect(String(putRes.body.error || '')).toContain('MM-DD');
      expect(String(putRes.body.error || '')).toContain('season_start_mmdd');
    });

    it('returns 400 when only one season boundary is provided', async () => {
      const putRes = await request(app)
        .put('/api/owner/settings/full')
        .set('Authorization', `Bearer ${ownerToken}`)
        .send({ season_start_mmdd: '05-01' });

      expect(putRes.status).toBe(400);
      expect(putRes.body.ok).toBe(false);
      expect(String(putRes.body.error || '')).toContain('обе границы');
    });

    it('returns 400 for cross-year MM-DD range (start > end)', async () => {
      const putRes = await request(app)
        .put('/api/owner/settings/full')
        .set('Authorization', `Bearer ${ownerToken}`)
        .send({ season_start_mmdd: '11-01', season_end_mmdd: '03-31' });

      expect(putRes.status).toBe(400);
      expect(putRes.body.ok).toBe(false);
      expect(String(putRes.body.error || '')).toContain('внутри одного года');
      expect(String(putRes.body.error || '')).toContain('season_start_mmdd');
      expect(String(putRes.body.error || '')).toContain('season_end_mmdd');
    });

    it('keeps MM-DD boundaries optional when not provided', async () => {
      await request(app)
        .put('/api/owner/settings/full')
        .set('Authorization', `Bearer ${ownerToken}`)
      ...
      expect(getRes.status).toBe(200);
      expect(getRes.body.data.season_start_mmdd).toBeUndefined();
      expect(getRes.body.data.season_end_mmdd).toBeUndefined();
    });
  });
});
